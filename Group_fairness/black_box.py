...
class Blackbox:
    ...
    def get_acc(self, inputs, targets):
        ...
        with torch.no_grad():
            outputs = self.net(inputs)
            acc = torch.sum(torch.where(torch.argmax(outputs, dim=1) == targets, 1.0, 0.0)) / outputs.shape[0]
...
class FaReG_Train:
    def __init__(self, net, device, train_loader, batch_size, max_epoch, learning_rate, optimizer,
                 val_loader, verbose = True):
        self.net = net.to(device)
        self.device = device
        self.batch_size = batch_size
    ...
    def condition_criterion(self, outputs, covs):
        return torch.sum(outputs.squeeze() * covs)/torch.sum(outputs)

    def vae_condition_criterion(self, outputs, covs, mu, logvar):
        miscov_loss = torch.sum(outputs.squeeze() * covs)/torch.sum(outputs)
        KLD = - 0.5 * torch.sum(1 + logvar - mu.pow(2) - logvar.exp())

        return miscov_loss + 2.0 * KLD
    ...
    def train_single_epoch(self):
        ...
        for i, (inputs, covs) in enumerate(self.train_loader):
            # Move tensors to correct device
            inputs, covs = inputs.to(self.device), covs.to(self.device)
            # zero the parameter gradients
            self.optimizer.zero_grad()

            outputs, mu, logvar = self.net(inputs)
            
            loss = self.vae_condition_criterion(outputs, covs, mu, logvar)
            loss.backward()
            self.optimizer.step()
    ...
    def full_train(self, save_dir = './models'):
        ...
        for e in tqdm(range(1, self.max_epoch+1)):
            ...
            # scheduler.step()
            self.net.eval()
            for inputs, covs in self.val_loader:
              inputs, covs = inputs.to(self.device), covs.to(self.device)
              outputs, _, _ = self.net(inputs)
              val_loss = self.condition_criterion(outputs, covs)
            ...
            if epoch_val_loss < best_epoch_val_loss:
                ...
                print('Current Epoch, Val Loss: ', e, best_epoch_val_loss)


        return stats


class FaReG_Rec_Train:
    def __init__(self, fareg, net, device, train_loader, batch_size, max_epoch, learning_rate, criterion, optimizer,
                 val_loader, verbose = True):
        self.fareg = fareg
        self.net = net.to(device)
        self.device = device
    ...
    def train_single_epoch(self):
        ...
        for i, (inputs, targets) in enumerate(self.train_loader):
            # Move tensors to correct device
            inputs, targets = inputs.to(self.device), targets.to(self.device)
            inputs = self.fareg.get_z(inputs)
            # zero the parameter gradients
            self.optimizer.zero_grad()
